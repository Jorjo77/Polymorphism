...
namespace Polymorphism
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // От ляво слагаме(работим с) базовия клас/интерфейс, а отдясно може да бъдат всички наследници на базовия клас/интерфейс-a! Така много лесно може да се променя функционалността!(с различен инпут, baseClass.Area() прави различни неща)
            ...
            //за да използваме добре полиморфизма трвбва да използваме по базови интерфейси като IEnumerable - позволява ни да имаме повече колекции вътре (съдържа List, Dictionary, Stack, Queue). Да свикваме да използваме най-базовите класове/интерфейси за постигане на добър полиморфизъм.
            ...
            //Design patterns са темплейти (шаблони) с които програмистите работят за да се постигне по-четим и лесен за поддръжка код. Factory Design pattern е такова нещо демо във Vehicles ! Идеята е че това е клас с метод отговорен за създаването на фаден обект (клас Factory).
            ...
            //Съвет: Четем си условието 2, 3 пъти, докато не добием в главата си реална картинка, т.е. целта на ООП е да пресъздаваме реални действия, пропъртита и бихейвиър на даден обект. След това да започнем да мислим обектно (имаме този обект, тоя. тоя комуникира с тоя). Добра идея е да разписваме на един ноутпад (пейнт за визуализация) и като си измислим структурата, първо правим нея, после методи с бихейвиър, а Енджина накрая вече.
